 ...
 // Fetch all countries list
   public static function getCountries() {
     try {
       ...
       while($resultSet = mysqli_fetch_assoc($result)) {
        $res[$resultSet['id']] = $resultSet['COUNTRY_NAME'];
       }
  ...
  public static function getStates($countryId) {
     try {
       ...
       $res = array();
       while($resultSet = mysqli_fetch_assoc($result)) {
        $res[$resultSet['id']] = $resultSet['STATE_NAME'];
       }
       $data = array('status'=>'success', 'tp'=>1, 'msg'=>"States fetched successfully.", 'result'=>$res);
     } catch (Exception $e) {
       $data = array('status'=>'error 2', 'tp'=>0, 'msg'=>$e->getMessage());
     } finally {
        return $data;
     }
   }
 ...
 // Fetch all cities list by state id
  public static function getCities($stateId) {
     try {
       ...
       $data = array('status'=>'error 1', 'tp'=>0, 'msg'=>$e->getMessage());
     } finally {
        return $data;
     }
   }   
